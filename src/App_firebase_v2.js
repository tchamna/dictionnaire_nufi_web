...
const App = () => {
  ...
  const [filteredWords, setFilteredWords] = useState([]); // Filtered words based on input
  const [wordsInSentence, setWordsInSentence] = useState([]); // State to store words from the input sentence
  ...
  const handleInputChange = (event) => {
    ...
    const words = transformedInput.split(/\s+/);
    setWordsInSentence(words);

    // Filter words from the database as user types
    const matchingWords = wordsData.filter((item) =>
      words.some((word) => cleanWord(item.word) === cleanWord(word))
    );
  ...
  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      // When user presses Enter, select the first matched word if available
      if (filteredWords.length > 0) {
        setSelectedWord(filteredWords[0]);
      } else {
        // Clear selection if no matching word is found
        setSelectedWord(null);
      }
    }
  };
  ...
  const onWordDoubleClick = (wordText) => {
    const word = wordsData.find(item => item.word.toLowerCase() === cleanWord(wordText));
    if (word) {
  ...
  const renderSentenceWords = () => {
    return wordsInSentence.map((word, index) => {
      const clean = cleanWord(word);
      const isAudioAvailable = audioMap[clean];
      const isDefinitionAvailable = wordsData.some((item) => item.word.toLowerCase() === clean);

      return (
        <span
          key={index}
          onClick={() => isAudioAvailable && playAudio(clean)}
          onDoubleClick={() => isDefinitionAvailable && onWordDoubleClick(clean)}
          className={`${
            isAudioAvailable || isDefinitionAvailable
  ...
  return (
    <div className="min-h-screen bg-white text-gray-800 dark:text-gray-100">
      ...
      <main className="p-6 flex flex-col lg:flex-row justify-center space-y-6 lg:space-y-0 lg:space-x-6">
        {/* Display word details if a matching word is found */}
        {selectedWord ? (
          <WordDetails
            word={selectedWord}
          ...
          // Display entered sentence if no specific word is selected
          searchQuery && (
            <div className="w-full lg:w-2/3 p-6 bg-white dark:bg-gray-800 shadow-xl rounded-lg">
              <p className="text-2xl">Entered Sentence:</p>
              <div className="mt-4 text-3xl font-medium">
                {searchQuery}
              </div>
              <p className="text-2xl mt-6">Click or double-click words:</p>
              <div className="flex flex-wrap mt-4">
                {renderSentenceWords()}
              </div>
              {/* Display filtered words from the database */}
              {filteredWords.length > 0 && (
                <div className="mt-6">
                  <h2 className="text-2xl font-bold mb-2">Matching Words from Database:</h2>
                  <ul>
